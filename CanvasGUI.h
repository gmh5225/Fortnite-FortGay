...
namespace CanvasGui
{
	...
	namespace Input
	{
		...
		void Handle()
		{
			if (GetAsyncKeyState(0x01)) //This is in bytes, you could change it to VK_INSERT
				mouseDown[0] = true;
			else
				mouseDown[0] = false;
		}
	...
	namespace PostRenderer //idk why i named it postrender
	{
		struct DrawList
		{
	...
	void TextLeft(const char* name, FVector2D pos, FLinearColor color, bool outline)
	{
		...
		Engine_R::RK2_DrawText(FString(s2wc(name)), pos, FVector2D{ 0.97f, 0.97f }, color, false, Colors::Text_Shadow, FVector2D{ pos.X + 1, pos.Y + 1 }, false, true, true, Colors::Text_Outline);
	}
	void TextCenter(const char* name, FVector2D pos, FLinearColor color, bool outline)
	{
		int length = strlen(name) + 1;
		Engine_R::RK2_DrawText(FString(s2wc(name)), pos, FVector2D{ 0.97f, 0.97f }, color, false, Colors::Text_Shadow, FVector2D{ pos.X + 1, pos.Y + 1 }, true, true, true, Colors::Text_Outline);
	}
	...
	void Draw_Line(FVector2D from, FVector2D to, int thickness, FLinearColor color)
	{
		K2_DrawLine(canvas, FVector2D{ from.X, from.Y }, FVector2D{ to.X, to.Y }, color, thickness);
	}
	void drawFilledRect(FVector2D initial_pos, float w, float h, FLinearColor color)
	{
		for (float i = 0.0f; i < h; i += 1.0f)
			K2_DrawLine(canvas, FVector2D{ initial_pos.X, initial_pos.Y + i }, FVector2D{ initial_pos.X + w, initial_pos.Y + i }, color, 1.0f);
	}
	...
	void Hotkey(char* name, FVector2D size, int* key)
	{
		...
		if (active_hotkey == elements_count)
		{
			...
			if (!CanvasGui::Input::IsAnyMouseDown())
			{
	...
	void Render()
	{
		for (int i = 0; i < 128; i++)
		{
			if (PostRenderer::drawlist[i].type != -1)
			{
				...
				if (PostRenderer::drawlist[i].type == 1)
				{
					CanvasGui::drawFilledRect(PostRenderer::drawlist[i].pos, PostRenderer::drawlist[i].size.X, PostRenderer::drawlist[i].size.Y, PostRenderer::drawlist[i].color);
				}
				//TextLeft
				else if (PostRenderer::drawlist[i].type == 2)
				{
					CanvasGui::TextLeft(PostRenderer::drawlist[i].name, PostRenderer::drawlist[i].pos, PostRenderer::drawlist[i].color, PostRenderer::drawlist[i].outline);
				}
				//TextCenter
				else if (PostRenderer::drawlist[i].type == 3)
				{
					CanvasGui::TextCenter(PostRenderer::drawlist[i].name, PostRenderer::drawlist[i].pos, PostRenderer::drawlist[i].color, PostRenderer::drawlist[i].outline);
				}
				//Draw_Line
