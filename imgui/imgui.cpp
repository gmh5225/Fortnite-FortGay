...
bool ImGui::IsHovered(const char* v, int w, int h) {
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	const ImVec2 label_size = ImGui::CalcTextSize(v, NULL, true);

	ImVec2 pos = window->DC.CursorPos;
	if (style.FramePadding.y < window->DC.CurrentLineTextBaseOffset) // Try to vertically align buttons that are smaller/have no padding so that text baseline matches (bit hacky, since it shouldn't be a flag)
		pos.y += window->DC.CurrentLineTextBaseOffset - style.FramePadding.y;
	ImVec2 size = ImGui::CalcItemSize(ImVec2(w, h), label_size.x + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 2.0f);
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
{
	...
	const float slider_usable_pos_min = (is_horizontal ? frame_bb.Min.x + 75 : frame_bb.Min.y) + grab_padding + grab_sz * 0.5f;
	...
	if (is_horizontal)
		...
		grab_bb = ImRect(ImVec2(frame_bb.Min.x + grab_padding + 75, grab_pos - grab_sz * 0.5f), ImVec2(frame_bb.Max.x - grab_padding, grab_pos + grab_sz * 0.5f));

	const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);

	window->DrawList->AddLine(ImVec2(frame_bb.Min.x + 75, frame_bb.Min.y + 10), ImVec2(frame_bb.Max.x, frame_bb.Min.y + 10), ImColor(153, 153, 153), 5);
	window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, ImColor(0, 0, 0), style.GrabRounding);
...
{
	char value_bufx[64];
	const char* value_buf_endx = value_bufx + ImFormatString(value_bufx, IM_ARRAYSIZE(value_bufx), "%.1f", v_max);

	char value_bufdx[64];
	const char* value_buf_endxd = value_bufdx + ImFormatString(value_bufdx, IM_ARRAYSIZE(value_bufdx), "%.1f", v_min);
	...
	char value_buf[64];
	const char* value_buf_end = value_buf + ImFormatString(value_buf, IM_ARRAYSIZE(value_buf), "%.1f", *v);
	...
	/*if (nigga)
	{
		char value_buf2[64] = " (deg)";

		if (label_size.x > 0.0f) {
			RenderText(ImVec2(frame_bb.Max.x - style.FramePadding.x - 180, frame_bb.Min.y - 15.f), label);
			RenderText(ImVec2(frame_bb.Max.x + style.FramePadding.x - 30, frame_bb.Min.y - 15.f), value_buf);
			RenderText(ImVec2(frame_bb.Max.x + style.FramePadding.x + 6, frame_bb.Min.y - 15.f), value_buf2);
		}
	}
	else*/
	//	{
	if (label_size.x > 0.0f) {
		RenderText(ImVec2(frame_bb.Max.x - style.FramePadding.x - 130, frame_bb.Min.y - 15.f), label);
		RenderText(ImVec2(frame_bb.Max.x + style.FramePadding.x - 30, frame_bb.Min.y + 15.f), value_bufx);
		RenderText(ImVec2(frame_bb.Max.x - style.FramePadding.x - 130, frame_bb.Min.y + 15.f), value_bufdx);
		RenderText(ImVec2(grab_bb.Max.x, grab_bb.Min.y + 15.f), value_buf);
	}
...
//   "%5.2f secs"   01.23 secs
//   "Gold: %.0f"   Gold: 1
bool ImGui::SliderFloat22(const char* label, float* v, float v_min, float v_max, const char* display_format, float power)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	// Actual slider behavior + render grab
	ItemSize(total_bb, style.FramePadding.y);
	const bool value_changed = SliderBehavior2(frame_bb, id, v, v_min, v_max, power, decimal_precision);
...
bool ImGui::AppolSliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
{
	ImGuiContext& g = *GImGui;
	ImGuiWindow* window = GetCurrentWindow();
	const ImGuiStyle& style = g.Style;
	...
	window->DrawList->AddLine(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 10), ImVec2(frame_bb.Max.x, frame_bb.Min.y + 10), ImColor(227, 227, 227), 5);
	window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, ImColor(0, 0, 0), style.GrabRounding);

	return value_changed;
}

bool ImGui::AppolSliderFloat(const char* label, float* v, float v_min, float v_max, const char* display_format, float power, bool nigga)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	// Actual slider behavior + render grab
	ItemSize(total_bb, style.FramePadding.y);
	const bool value_changed = AppolSliderBehavior(frame_bb, id, v, v_min, v_max, power, decimal_precision);
